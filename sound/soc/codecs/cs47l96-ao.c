...
#define DRV_NAME "cs47l96-ao-codec"
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	reg = snd_soc_component_read32(comp, TACNA_AO_INPUT_CONTROL);
			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		priv->in_up_pending--;
		snd_soc_component_update_bits(comp, reg, TACNA_IN1LAO_MUTE, 0);
	...
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_update_bits(comp, reg, TACNA_IN1LAO_MUTE,
					      TACNA_IN1LAO_MUTE);
...
static int cs47l96_ao_component_probe(struct snd_soc_component *comp)
{
	struct cs47l96_ao *cs47l96_ao = snd_soc_component_get_drvdata(comp);
	struct tacna_priv *priv = &cs47l96_ao->core;
	...
	snd_soc_component_init_regmap(comp, cs47l96_ao->core.tacna->regmap);
...
static void cs47l96_ao_component_remove(struct snd_soc_component *comp)
{
	struct cs47l96_ao *cs47l96_ao = snd_soc_component_get_drvdata(comp);
	struct tacna *tacna = cs47l96_ao->core.tacna;

	wm_adsp2_component_remove(&cs47l96_ao->core.dsp[0], comp);

	tacna->dapm = NULL;
}

static int cs47l96_ao_set_fll(struct snd_soc_component *comp, int fll_id,
...
static struct platform_driver cs47l96_ao_component_driver = {
	.driver = {
...
module_platform_driver(cs47l96_ao_component_driver);
