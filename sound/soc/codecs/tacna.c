		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	reg = snd_soc_component_read32(comp, TACNA_INPUT_CONTROL);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	len = params->num_regs * comp->val_bytes;
		       ...
		       struct snd_ctl_elem_value *ucontrol)
{
	...
	val = snd_soc_component_read32(comp, mc->reg);
		  ...
		  struct snd_ctl_elem_value *ucontrol)
{
	...
	/* Cannot change dfc settings when its on */
	val = snd_soc_component_read32(comp, reg);
	if (val & TACNA_DFC1_CH1_EN) {
		ret = -EBUSY;
		goto exit;
	}
...
ovf_err:
	dev_err(comp->dev, "%s %cX rate control exceeds rate array\n",
			  ...
			  unsigned int freqsel_reg)
{
	...
	if (ret) {
		dev_err(comp->dev, "Failed to read 0x%x: %d\n", freq_reg, ret);
		return ret;
	}
	...
	ret = regmap_write(dsp->regmap, dsp->base + TACNA_DSP_CLOCK_FREQ_OFFS,
			   freq);
	if (ret) {
		dev_err(comp->dev, "Failed to set HALO clock freq: %d\n", ret);
		return ret;
	}
...
static int tacna_set_opclk(struct snd_soc_component *comp, unsigned int clk,
			   unsigned int freq)
{
	struct tacna_priv *priv = snd_soc_component_get_drvdata(comp);
	unsigned int reg;
	...
	for (ref = 0; ref < ARRAY_SIZE(tacna_opclk_ref_48k_rates); ++ref) {
		...
		while ((rates[ref] / div >= freq) && (div <= 30)) {
			if (rates[ref] / div == freq) {
				dev_dbg(comp->dev, "Configured %dHz OPCLK\n",
					freq);
	...
	dev_err(comp->dev, "Unable to generate %dHz OPCLK\n", freq);
	return -EINVAL;
}

static int tacna_set_dacclk(struct snd_soc_component *comp, int source,
			    unsigned int freq)
{
	struct tacna_priv *priv = snd_soc_component_get_drvdata(comp);
...
static int tacna_set_pdm_fllclk(struct snd_soc_component *comp, int source)
{
	struct tacna_priv *priv = snd_soc_component_get_drvdata(comp);
	struct tacna *tacna = priv->tacna;
	unsigned int val;
...
int tacna_set_sysclk(struct snd_soc_component *comp, int clk_id, int source,
		     unsigned int freq, int dir)
{
	...
	case TACNA_CLK_OPCLK:
	case TACNA_CLK_ASYNC_OPCLK:
		return tacna_set_opclk(comp, clk_id, freq);
	case TACNA_CLK_DACCLK:
		return tacna_set_dacclk(comp, source, freq);
	...
	case TACNA_CLK_PDM_FLLCLK:
		return tacna_set_pdm_fllclk(comp, source);
	default:
		return -EINVAL;
	}
...
static bool tacna_asp_cfg_changed(struct snd_soc_component *comp,
				  unsigned int base, unsigned int sclk,
				  unsigned int slotws, unsigned int dataw)
{
	unsigned int val;

	val = snd_soc_component_read32(comp, base + TACNA_ASP_CONTROL1);
	if (sclk != (val & TACNA_ASP1_BCLK_FREQ_MASK))
		return true;

	val = snd_soc_component_read32(comp, base + TACNA_ASP_CONTROL2);
	if (slotws != (val & (TACNA_ASP1_RX_WIDTH_MASK |
			      TACNA_ASP1_TX_WIDTH_MASK)))
		return true;

	val = snd_soc_component_read32(comp, base + TACNA_ASP_DATA_CONTROL1);
	if (dataw != (val & (TACNA_ASP1_TX_WL_MASK)))
		return true;

	val = snd_soc_component_read32(comp, base + TACNA_ASP_DATA_CONTROL5);
			   ...
			   struct snd_soc_dai *dai)
{
	...
	reconfig = tacna_asp_cfg_changed(comp, base, sclk, slotw, dataw);

	if (reconfig) {
		/* Save ASP TX/RX state */
		asp_state = snd_soc_component_read32(comp,
 ...
 */
int tacna_set_output_mode(struct snd_soc_component *comp, int output, bool diff)
{
	...
	ret = snd_soc_component_update_bits(comp, reg, TACNA_OUT1_MONO, val);
	if (ret < 0)
		return ret;
	else
		return 0;
...
int tacna_get_accdet_for_output(struct snd_soc_component *comp, int output)
{
	struct tacna *tacna = dev_get_drvdata(comp->dev->parent);
	int i;
		...
		int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		priv->in_up_pending--;
		snd_soc_component_update_bits(comp, reg, TACNA_IN1L_MUTE, 0);

		/* Uncached write-only register, no need for update_bits */
		if (!priv->in_up_pending)
			snd_soc_component_write(comp, priv->in_vu_reg,
						TACNA_IN_VU);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_update_bits(comp, reg,
		       ...
		       struct snd_ctl_elem_value *ucontrol)
{
	...
	/*
	 * Uncached write-only register, no need for update_bits.
	 * Will fail if component is off but that will be handled by tacna_in_ev
	 */
	snd_soc_component_write(comp, priv->in_vu_reg, TACNA_IN_VU);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	 ...
	 */
	accdet = tacna_get_accdet_for_output(comp, out_num);
	if (accdet >= 0 &&
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_context *dapm =
					snd_soc_dapm_kcontrol_dapm(kcontrol);
	struct snd_soc_component *comp = snd_soc_dapm_kcontrol_component(kcontrol);
	struct tacna_priv *priv = dev_get_drvdata(comp->dev);
	struct tacna *tacna = priv->tacna;
	struct soc_enum *e = (struct soc_enum *)kcontrol->private_value;
	...
	if (!snd_soc_component_test_bits(comp, e->reg, mask, hp2_sel)) {
		snd_soc_dapm_mutex_unlock(dapm);
		return 0;
	}
	...
	if (ret)
		dev_warn(comp->dev, "Failed to read OUTPUT_ENABLE_1: %d\n",
			 ret);
	...
	ret = regmap_update_bits_check(tacna->regmap, TACNA_OUTPUT_ENABLE_1,
				       ...
				       0, &change);
	if (ret) {
		dev_warn(comp->dev, "Failed to disable outputs: %d\n", ret);
	} else if (change) {
		tacna_wait_for_output_seq(priv,
	...
	ret = regmap_update_bits_check(tacna->regmap, TACNA_OUTPUT_ENABLE_1,
				       ...
				       cur, &change);
	if (ret) {
		dev_warn(comp->dev, "Failed to restore outputs: %d\n", ret);
	} else if (change) {
		tacna_wait_for_output_seq(priv,
					  TACNA_OUT1L_STS | TACNA_OUT1R_STS,
					  cur);
		 ...
		 int event)
{
	...
	snd_soc_component_write(comp, TACNA_ANC_CTRL_1, val);
...
int tacna_init_inputs(struct snd_soc_component *comp)
{
	struct tacna_priv *priv = snd_soc_component_get_drvdata(comp);
	struct tacna *tacna = priv->tacna;
...
int tacna_init_auxpdm(struct snd_soc_component *comp, int n_auxpdm)
{
	struct tacna_priv *priv = snd_soc_component_get_drvdata(comp);
	struct tacna *tacna = priv->tacna;
...
int tacna_init_outputs(struct snd_soc_component *comp,
