...
TRACE_EVENT(clsic_vox_trigger_heard,
	...
	TP_printk("end of triggering with %s %d",
		  (__entry->msgid == CLSIC_VOX_MSG_N_LISTEN_ERR) ? "error" :
			"biometric TD|TI flags",
		  __entry->flags)
);
...
TRACE_EVENT(clsic_vox_asr_stream_listen,
	...
	TP_ARGS(trg_domain),
	TP_STRUCT__entry(
		__field(enum clsic_vox_trgr_domain, trg_domain)
	),
	TP_fast_assign(
		__entry->trg_domain = trg_domain;
	),
...
TRACE_EVENT(clsic_vox_set_mode,
	TP_PROTO(
		enum clsic_vox_mode initial_mode,
...
TRACE_EVENT(clsic_vox_set_drv_state,
	...
	TP_printk("CLSIC change driver state from %d to %d",
...
TRACE_EVENT(clsic_vox_start_enrol_user,
	...
	TP_printk("enrol user %u for phrase %u with timeout %ums",
		  __entry->user_id,
		  __entry->phrase_id,
		  __entry->timeout)
);
