...
err:
	...
	dev_warn(comp->dev, "Failed to get OUTH CP disabled (%d)\n", ret);
	return ret;
}
			   ...
			   struct snd_kcontrol *kcontrol, int event)
{
	 ...
	 */
	accdet = tacna_get_accdet_for_output(comp, 1);
	if (accdet >= 0 &&
	...
	if (ret)
		dev_err(comp->dev, "Failed to toggle OUTH enable: %d\n", ret);
	...
	if (outh_upd) { /* wait for enable/disable to take effect */
		...
		if (!time_left)
			dev_warn(comp->dev, "OUTH %s timed out.\n",
				 (val) ? "enable" : "disable");
	}
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	struct cs47l96 *cs47l96 = dev_get_drvdata(comp->dev);
	struct tacna *tacna = cs47l96->core.tacna;
	...
	if (!snd_soc_test_bits(comp, e->reg, mask, hp2_sel)) {
	...
	if (tacna_get_accdet_for_output(comp, 1) < 0) {
		...
		/*cur = tacna->hp_ena;*/
		dev_warn(comp->dev, "OUT1 demux handling of accdet incomplete\n");
	}
...
static int cs47l96_component_probe(struct snd_soc_component *comp)
{
	struct cs47l96 *cs47l96 = snd_soc_component_get_drvdata(comp);
	struct tacna *tacna = cs47l96->core.tacna;
	int ret;

	cs47l96->core.tacna->dapm = snd_soc_component_get_dapm(comp);
	...
	ret = tacna_init_auxpdm(comp, CS47L96_N_AUXPDM);
	if (ret)
		return ret;

	ret = tacna_init_outputs(comp, CS47L96_MONO_OUTPUTS);
	if (ret)
		return ret;
	...
	ret = snd_soc_add_component_controls(comp, tacna_dsp_rate_controls,
					     CS47L96_DSP_N_RX_CHANNELS +
					     CS47L96_DSP_N_TX_CHANNELS);
	if (ret)
		return ret;

	wm_adsp2_component_probe(&cs47l96->core.dsp[0], comp, false);

	return 0;
}

static void cs47l96_component_remove(struct snd_soc_component *comp)
{
	struct cs47l96 *cs47l96 = snd_soc_component_get_drvdata(comp);
	struct tacna *tacna = cs47l96->core.tacna;
...
static int cs47l96_set_fll(struct snd_soc_component *comp, int fll_id, int source,
...
static int cs47l96_probe(struct platform_device *pdev)
{
	...
	ret = devm_snd_soc_register_component(&pdev->dev,
					      &soc_component_dev_cs47l96,
					      cs47l96_dai,
					      ARRAY_SIZE(cs47l96_dai));
	if (ret < 0) {
...
static struct platform_driver cs47l96_component_driver = {
	.driver = {
		.name = "cs47l96-codec",
		.owner = THIS_MODULE,
	},
